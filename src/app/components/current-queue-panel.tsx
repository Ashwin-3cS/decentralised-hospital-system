...
const getStatusColor = (status: string) => {
  switch (status) {
    case 'Waiting': return 'bg-[#666666]'
    case 'Active': return 'bg-[#2E7D32]'
    case 'Emergency': return 'bg-[#D32F2F]'
    case 'Almost': return 'bg-[#ED6C02]'
    ...
    <Card className="w-full">
      ...
      <CardContent>
        <div className="flex flex-col items-center space-y-4">
          ...
          <Badge className={`${getStatusColor(status)} text-white px-4 py-2 text-lg`}>
            {status}
          </Badge>
        </div>
      </CardContent>
