      ...
      {searchResults.length > 0 && (
        <div>
          ...
          {searchResults.map((patient) => (
            <div key={patient.id} className="bg-gray-100 p-4 rounded-md mb-2 flex justify-between items-center">
              <div>
                <p className="font-semibold">{patient.name}</p>
                <p className="text-sm text-gray-600">ID: {patient.id}</p>
                <p className="text-sm text-gray-600">Access: {patient.accessStatus}</p>
              </div>
