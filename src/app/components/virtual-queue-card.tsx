        ...
        {!isInQueue ? (
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" className="w-full" disabled={isLoading}>
              {isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : 'Join Queue'}
            </Button>
          </form>
        ) : (
          <div className="space-y-4">
            <div className="text-center">
              <p className="text-2xl font-bold">Your position: #{queuePosition}</p>
            ...
            <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${(currentServingNumber / (currentServingNumber + queuePosition)) * 100}%` }}
              ></div>
            </div>
